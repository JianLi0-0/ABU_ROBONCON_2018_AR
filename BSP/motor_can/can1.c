...
set_point      		单位(正负代表不同的方向)

	under Position_Mode    counts             若编码器线数为1000(四倍频后4000 counts),则setpoint 4000表示转一圈
	under Velocity_Mode    counts/s						若编码器线数为1000(四倍频后4000 counts),则setpoint 4000表示目标速度为1s转一圈
	under Current_Mode     mA									目标电流，乘上电机的转矩常数则为扭矩,最小分辨率为10ma
...
void Odrive_Control(CAN_TypeDef *CANx, u8 motor_num, int set_point, u8 control_mode)
{
	...
	tx_message.StdId = (motor_num << 5) + (0x00B + control_mode); //高五位为电机编号，低六位为控制模式，0x0C,0x0D,0x0E分别是位置 速度 电流模式，对应control_mode为1，2，3
...
void GET_LIDAR_MSG(CanRxMsg * msg)
{
	...
	switch(msg->StdId)
	{
		case LIDAR_XY_MSG_ID:
		{
			...
			LIDAR_POSITION.X_POS = LIDAR_R_POS_UNION.ActVal[0];   //实际安装与原设定坐标旋转了180度，加负号
		...
		case LIDAR_ANGLE_MSG_ID:
		{
			...
			LIDAR_POSITION.ANGLE_POS = LIDAR_R_POS_UNION.ActVal[2];  //使用陀螺仪返回角度
		};break;
